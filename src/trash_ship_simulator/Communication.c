...
uint8_t handleNewClient(Player **firstPlayer, int *nNewPlayers, GameState *game_state, uint8_t *spaceships, gful_lifo *graceful_lifo){
    if(game_state->n_ships == 0){
        return newPLayerFirstPlace(firstPlayer, nNewPlayers, game_state, spaceships, graceful_lifo);

    }else if(game_state->n_ships < game_state->n_planets){
        return newPlayerOtherPlace(*firstPlayer, game_state, spaceships);
    ...
    uint8_t newPLayerFirstPlace(Player **firstPlayer, int *nNewPlayers, GameState *game_state, uint8_t *spaceships, gful_lifo *graceful_lifo){
        game_state->n_ships++;
        (*nNewPlayers)--;
        spaceships[0] = 1;
    ...
    uint8_t newPlayerOtherPlace(Player *firstPlayer, GameState *game_state, uint8_t *spaceships){
        //Find unused iD
        uint8_t newID = (uint8_t)(rand() % TOTALCOMBINATIONS);
        Player * currPlayer;
...
uint8_t handleClientExit(Player **firstPlayer, uint8_t msgSenderID, int *nNewPlayers, uint8_t *spaceships, GameState *game_state){
